...
Strategy version 1.1 supports multiple daily Yahoo and BLS sources. Version 1.2
supports regular-session one-hour Yahoo bars. The local integration worktree
includes both versions in the frozen frontend contract; promotion to `main` is
pending. Version 1.2 stored results use UTC timestamps in equity and trade date
fields and round-trip through `GET /strategies/{strategy_id}`.
